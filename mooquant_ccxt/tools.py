...
from mooquant_ccxt import barfeed


def download_csv(broker, instrument, begin, end):
    ex = getattr(ccxt, broker)()
    return ts.loadMarkets(broker, instrument, begin, end)


def download_daily_bars(broker, instrument, year, csvFile):
    ...
    bars = download_csv(broker, instrument, begin, end)
...
def build_feed(broker, instruments, fromYear, toYear, storage, frequency=bar.Frequency.DAY, skipErrors=False):
    """Build and load a :class:`mooquant.barfeed.ccxtfeed.Feed` using CSV files downloaded from Google Finance.
    ...
    :rtype: :class:`mooquant.barfeed.ccxtfeed.Feed`.
    """

    logger = mooquant.logger.getLogger("mooquant_ccxt")
    ...
    for year in range(fromYear, toYear + 1):
        for instrument in instruments:
            fileName = os.path.join(
                storage,
                "{instrument}-{year}-ccxt.csv".format(
